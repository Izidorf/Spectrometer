...
class Spectrometer:
	...
	def plotSpectra(self):
		brighestRow = self.find_brightest_row()
		sp = self.get_spectra(brighestRow)
		figure = self.plot_spectral_row(sp)
		return figure
	...
	def plot_spectral_row(self, specter):
		'requires pylab from matplotlib'
		size = len(specter[self.R])
		fig = figure()
		plt.subplot(212)
		plot(np.linspace(self.left,self.right,size), specter[self.R], 'r-',label='red')
		plot(np.linspace(self.left,self.right,size), specter[self.G], 'g-',label='green')
		plot(np.linspace(self.left,self.right,size), specter[self.B], 'b-',label='blue')
		plot(np.linspace(self.left,self.right,size), specter[self.A], 'k-',label='average')
		...
		subplot(211)
		imshow(self.image)
		plot(np.linspace(0, self.image.size[self.R],self.image.size[self.R]),[self.samplingRow]*self.image.size[self.R], 'y-') #draw a sampling row
